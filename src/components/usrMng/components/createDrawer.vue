<template>
    <Drawer title="添加成员" v-model="value" width="500" :mask-closable="false" :styles="styles">
...
<!--                </Select>-->
<!--            </FormItem>-->
            <FormItem label="职位" prop="rank">
                <Input v-model="formData.rank" placeholder="请输入职位 (选填)" />
            </FormItem>
            <FormItem label="地址" prop="address">
                <Input v-model="formData.address" placeholder="请输入所在地 (选填)" />
            </FormItem>
            <FormItem label="介绍" prop="introduction">
                <Input v-model="formData.introduction" placeholder="请输入介绍（公司）" />
            </FormItem>
            <FormItem label="研究兴趣" prop="hobby">
                <Input v-model="formData.hobby" placeholder="请输入兴趣 (选填)" />
            </FormItem>
        </Form>
...
export default {
    ...
    methods: {
        ...
        adduser () {
            ...
            this.axios.post('/api/user/addMember', JSON.stringify(this.formData), {
                headers: {'Content-Type': 'application/json;charset=UTF-8'}
            }).then((res) => {
                console.log(res)
                if (res.data.code === 200) {
                    this.$Message.success('新成员创建成功！')
                } else {
        ...
        updateuser () {
            ...
            delete form._index
            delete form._rowKey
            this.axios.post('/api/user/updateMember', JSON.stringify(this.formData), {
              headers: {'Content-Type': 'application/json;charset=UTF-8'}
                }).then(result => {
              // res.data.data

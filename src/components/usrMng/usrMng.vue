...
<div class="usrMng">
    ...
    <div class="usrOperate">
        <Button type="primary" @click="addUser()">添加成员</Button>
<!--        <div class="usrSearch">-->
          ...
          <template slot-scope="{ row, index }" slot="uploadPic">
            <div>
                <Upload
                  ...
                  action="//121.40.178.147:11180/xlab/user/addImage">
...
export default {
    ...
    data () {
        return {
            ...
            searchVal: '',  // 输入搜索值
            list: [
                {title: '中文名', key: 'nameCh', width: 100},
                {title: '英文名', key: 'nameEn', width: 120},
                {title: 'email', key: 'email'},
                {title: '电话', key: 'phone', width: 170},
                {title: '职位', key: 'rank', width: 120},
    ...
    methods: {
      ...
      submit: function (event,row,index) {
        //阻止元素发生默认的行为
        console.log(row.id)
        ...
        formData.append("file", this.file,"user" + row.id +".jpg");
        console.log(formData);
        this.axios.post('/api/user/addImage', formData)
      ...
      /**
         ...
         */
        getUserList () {
            this.axios.post('/api/user/findAllMember', {
      ...
      /**
        ...
        remove (row, index) {
            return new Promise(() => {
                this.$Modal.confirm({
                    title: '确认删除',
                    content: '您确认要删除该成员信息吗？',
                    onOk: () => {
                        if (row.accountLevel === 1111) {
                            this.$Message.info('不可删除管理员用户！')
                        } else {
                            this.axios.post('/api/user/delMember', {
                                'id': row.id,
                                'token': this.token
                            }).then((res) => {
